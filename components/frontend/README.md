...
- `GITHUB_CALLBACK_URL` (optional)
  - Explicit callback URL for GitHub App OAuth. Used when multiple clusters share one GitHub App. Falls back to `<current origin>/api/auth/github/user/callback`. Must be registered as a callback URL in the GitHub App settings. In production, set via `frontend-config` ConfigMap (key: `github-callback-url`).
